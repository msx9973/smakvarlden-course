...
export default function RecipeDetail({ id }: { id: number }) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      <EditRecipeDialog open={editOpen} recipe={r.id?{id:r.id,name:r.name,description:r.description,category:r.category,servings:r.servings,sellingPriceSek:r.sellingPriceSek,isShared:r.isShared}:null} onClose={()=>{setEditOpen(false);qc.invalidateQueries({queryKey:getGetRecipeQueryKey(id)});}} />
    </div>
  );
}
