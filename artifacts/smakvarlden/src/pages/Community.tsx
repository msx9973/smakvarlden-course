...
const schema = z.object({
  recipeName:  z.string().min(1, "Ange ett receptnamn"),
  chefName:    z.string().min(1, "Ange ditt namn"),
  description: z.string().min(10, "Beskriv receptet (minst 10 tecken)"),
  ...
  const create = useCreateCommunityPost({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: "Recept delat med communityn!" });
        onClose();
        form.reset();
      },
    },
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md rounded-2xl">
        <DialogHeader>
          <DialogTitle className="font-serif text-lg" style={{ color: "hsl(17 47% 13%)" }}>Dela ett recept</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit((data) => create.mutate({ data }))} className="space-y-4">
            <FormField control={form.control} name="recipeName" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(20 20% 58%)" }}>Receptnamn</FormLabel>
            ...
            <FormField control={form.control} name="chefName" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(20 20% 58%)" }}>Ditt namn</FormLabel>
                <FormControl><ShadInput {...field} className="rounded-xl" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <div className="grid grid-cols-2 gap-3">
              <FormField control={form.control} name="category" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(20 20% 58%)" }}>Kategori</FormLabel>
              ...
              <FormField control={form.control} name="costSek" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(20 20% 58%)" }}>Kostnad (kr)</FormLabel>
            ...
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(20 20% 58%)" }}>Beskrivning</FormLabel>
                <FormControl><Textarea rows={3} {...field} className="rounded-xl resize-none" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <DialogFooter className="gap-2">
              <Button type="button" variant="outline" onClick={onClose} className="rounded-full">Avbryt</Button>
              <Button type="submit" disabled={create.isPending} className="rounded-full">Dela recept</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold tracking-tight" style={{ color: "hsl(17 47% 13%)" }}>Community</h1>
          <p className="text-[13px] mt-1" style={{ color: "hsl(20 20% 58%)" }}>Recept delade av kockar i Sverige</p>
        </div>
        <button
          ...
          style={{
            ...
            color: "#FAF8F4",
            boxShadow: "0 4px 14px rgba(44,24,16,.22)",
          }}
        >
          <Plus className="w-4 h-4" /> Dela recept
        </button>
      </div>
      ...
      {posts.isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <p className="font-serif text-base font-semibold" style={{ color: "hsl(17 47% 13%)" }}>Inga inlägg hittades</p>
          <p className="text-[13px] mt-1" style={{ color: "hsl(20 20% 60%)" }}>Var den första att dela ett recept!</p>
        </div>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <div>
                      ...
                      <p className="text-[12px] mt-0.5" style={{ color: "hsl(20 20% 60%)" }}>
                        av <span className="font-semibold" style={{ color: "hsl(17 47% 20%)" }}>{post.chefName}</span>
                        {" · "}{formatDistanceToNow(new Date(post.createdAt), { addSuffix: true, locale: sv })}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      <div className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "hsl(44 54% 50%)" }}>Kostnad</div>
