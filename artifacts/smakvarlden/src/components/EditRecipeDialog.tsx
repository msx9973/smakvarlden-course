...
import { useUpdateRecipe } from "@workspace/api-client-react";
...
export function EditRecipeDialog({
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: { name: "", description: "", category: "", servings: 4, sellingPriceSek: 0 },
  });
  ...
  return (
    <Dialog open={open && !!recipe} onOpenChange={(next) => { if (!next) onClose(); }}>
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((data) => {
              if (recipe) update.mutate({ id: recipe.id, data });
            })}
            className="space-y-4"
          >
