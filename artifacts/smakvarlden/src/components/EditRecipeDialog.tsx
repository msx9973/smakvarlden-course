...
type EditableRecipe = Pick<
  Recipe,
  "id" | "name" | "description" | "category" | "servings" | "sellingPriceSek" | "isShared"
>;

type FormValues = z.infer<typeof schema>;

function defaults(recipe: EditableRecipe): FormValues {
  return {
    name: recipe.name,
...
export function EditRecipeDialog({
  ...
  useEffect(() => {
    if (recipe) form.reset(defaults(recipe));
  }, [form, recipe]);
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((data) => {
              if (!recipe) return;
              update.mutate({ id: recipe.id, data });
            })}
            className="space-y-4"
          >
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={!recipe || update.isPending}>{t("Spara")}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
