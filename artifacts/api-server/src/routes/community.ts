...
// NOTE: This in-memory cache works on long-running servers.
// On Netlify Functions (cold-start per request) it resets each invocation.
// For serverless deployments, replace with Netlify Blobs or an edge KV store.
...
router.get("/posts", async (req, res) => {
  ...
  const rows = await db
    ...
    .where(
      search
        ? ilike(communityPostsTable.recipeName, `%${search}%`)
        : undefined,
    )
...
router.get("/news", async (req, res) => {
  ...
  try {
    if (
      newsCache[cacheKey] &&
      newsCache[cacheKey].expiresAt > Date.now()
    ) {
      res.set(
        "Cache-Control",
    ...
    const items =
      lang === "en"
        ? await translateNewsToEnglish(swedishItems)
        : swedishItems;
    const fullyTranslated =
      lang === "sv" || items.every((item) => item.language === "en");
    newsCache[cacheKey] = {
      expiresAt:
        Date.now() + (fullyTranslated ? WEEK_MS : 10 * 60 * 1000),
      items,
    };
    ...
    return res.json(
      newsCache[cacheKey]?.items ?? newsCache["news:sv"]?.items ?? [],
    );
  }
});
...
router.post("/posts/:id/like", requireAuth, async (req, res) => {
  const parsed = LikeCommunityPostParams.safeParse({
...
async function translateNewsToEnglish(items: NewsItem[]) {
  ...
  try {
    const response = await client.messages.create({
      ...
      max_tokens: 4000,
      system:
        "Translate Swedish restaurant industry news into natural, concise English. Return only raw JSON with no markdown, no prose, and no code fences.",
      messages: [
        {
          role: "user",
          content: JSON.stringify({
            instruction:
              "Translate each title and summary to English. Preserve id, source, url, and publishedAt exactly. Return a JSON array.",
            items: items.map(
              ({ id, title, summary, source, url, publishedAt }) => ({
                id,
                title,
                summary,
                source,
...
async function fetchFeed(feed: {
  ...
  const response = await fetch(feed.url, {
    headers: {
      ...
      "User-Agent":
        "Smakvarlden/1.0 (+https://github.com/msx9973/smakvarlden-course)",
    },
  });
  ...
  return [...xml.matchAll(/<item\b[\s\S]*?<\/item>/gi)]
    .map((match) => {
      ...
      const summary = stripHtml(
        decodeXml(readTag(item, "description")),
      ).slice(0, 220);
