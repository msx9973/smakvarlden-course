...
function signToken(user: { id: number; email: string; role: string }) {
  return jwt.sign({ id: user.id, email: user.email, role: user.role }, getSecret(), {
    expiresIn: "14d", issuer: "smakvarlden", audience: "smakvarlden-app",
  });
}
...
function validatePassword(password: unknown) {
  ...
  if (!/[A-Za-zÅÄÖåäö]/.test(password) || !/\d/.test(password)) return "Lösenordet måste innehålla både bokstäver och siffror.";
  return null;
}

function verifyToken(token: string) {
  return jwt.verify(token, getSecret(), { issuer: "smakvarlden", audience: "smakvarlden-app" }) as { id: number };
}
...
router.get("/auth/google/callback", async (req, res) => {
  ...
  } catch { return res.redirect("/login?oauth=failed"); }
});
...
router.post("/auth/register", async (req, res) => {
  ...
  if (typeof name !== "string" || name.trim().length < 2 || !storageEmail || !password) return res.status(400).json({ error: "Fyll i namn, e-post eller telefonnummer och lösenord." });
