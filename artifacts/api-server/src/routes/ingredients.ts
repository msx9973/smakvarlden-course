...
function formatIngredient(row: typeof ingredientsTable.$inferSelect) {
  return { id: row.id, name: row.name, category: row.category, unit: row.unit, currentPriceSek: parseFloat(String(row.currentPriceSek)), priceChangePct: parseFloat(String(row.priceChangePct)), supplier: row.supplier ?? undefined, updatedAt: row.updatedAt.toISOString() };
}
...
async function recalcRecipesForIngredients(ingredientIds: number[]) {
  ...
  for (const { recipeId } of affected) {
    ...
    await db.update(recipesTable).set({ totalCostSek: String(totalCost), profitMarginPct: String(margin), updatedAt: new Date() }).where(eq(recipesTable.id, recipeId));
  }
}
...
router.post("/", async (req, res) => {
  ...
  await db.insert(activityLogTable).values({ type: "recipe_created", title: `Ny ingrediens tillagd: ${row.name}`, subtitle: `${row.category} · ${row.currentPriceSek} SEK/${row.unit}` });
...
router.get("/category-breakdown", async (_req, res) => {
  const rows = await db.select({ category: ingredientsTable.category, count: sql<number>`count(*)::int`, avgPriceSek: sql<number>`avg(current_price_sek::numeric)`, totalPriceSek: sql<number>`sum(current_price_sek::numeric)` }).from(ingredientsTable).groupBy(ingredientsTable.category);
...
router.put("/:id", requireAdmin, async (req, res) => {
  ...
  const [row] = await db.update(ingredientsTable).set({ name: bodyParsed.data.name ?? existing[0].name, category: bodyParsed.data.category ?? existing[0].category, unit: bodyParsed.data.unit ?? existing[0].unit, currentPriceSek: String(newPrice), priceChangePct: String(Math.round(changePct * 100) / 100), supplier: bodyParsed.data.supplier ?? existing[0].supplier, updatedAt: new Date() }).where(eq(ingredientsTable.id, paramParsed.data.id)).returning();
  if (Math.abs(changePct) > 0.01) await db.insert(activityLogTable).values({ type: "price_change", title: `Prisändring: ${row.name}`, subtitle: `${changePct > 0 ? "+" : ""}${Math.round(changePct * 10) / 10}% till ${newPrice} SEK/${row.unit}` });
...
router.delete("/:id", requireAdmin, async (req, res) => {
  ...
  const result = await db.transaction(async (tx) => {
    const [ingredient] = await tx
      ...
      .for("update");
    if (!ingredient) return "not-found" as const;

    const [usage] = await tx
      .select({ recipeId: recipeIngredientsTable.recipeId })
      .from(recipeIngredientsTable)
      .where(eq(recipeIngredientsTable.ingredientId, parsed.data.id))
      .limit(1);
    if (usage) return "in-use" as const;

    await tx.delete(ingredientsTable).where(eq(ingredientsTable.id, parsed.data.id));
    return "deleted" as const;
  });
  if (result === "not-found") return res.status(404).json({ error: "Not found" });
  if (result === "in-use") return res.status(409).json({ error: "Ingredient is used by one or more recipes" });
